...
No public application deployment or paid provisioning has been performed. The public GitHub repository is source publication only. Invitation beta readiness also requires live provider validation and operational sign-off.

## Chosen architecture

One Ubuntu LTS VM (4 GB RAM, 2 vCPU minimum for a small pilot) running Docker Compose: Caddy HTTPS reverse proxy, FastAPI serving the compiled React app, PostgreSQL 17 and one durable worker. Database and object volumes stay private; API binds to loopback. Encrypted off-host backups are required. A Hetzner shared VM is the reference option; another Docker-capable host can use the same configuration. The pilot starts with a volume-backed private object store. No S3 account, public bucket or mandatory cloud service is created.
...
Open `http://127.0.0.1:8000`. API health: `/api/health`. Local accounts are documented in README. `docker compose logs --tail=50 worker` reports worker startup; detailed job errors appear in the authenticated work item.
...
4. Use secure OpenAI Developers setup in Codex for a new key, or the organization's approved secret-management process. The user authorized secure key creation after local verification. Do not copy a key into chat. Set provider project usage caps/alerts in addition to Dove's per-work call allowance.
...
6. Put Caddy in front of the loopback API using `deploy/Caddyfile` and the production Compose override. Set `DOVE_DOMAIN` to the approved hostname. HTTPS cookies and write-origin checks require matching `PUBLIC_URL`.
...
- GPT-4.1 mini: $0.40 per million input tokens and $1.60 per million output tokens. At 100 work items/month × three analyses × 8,000 input and 1,000 output tokens, modeled inference is **$1.44/month**. Large documents/retries can raise this; it is not measured pilot cost. [Official model pricing](https://developers.openai.com/api/docs/models/gpt-4.1-mini).
- Resend lists 3,000 emails/month with a 100/day limit on Free, or $20/month for 50,000 emails on Pro; verify sending/receiving quotas and overages for the selected account before launch. Budget $0–$20/month for this small pilot, with no guarantee a free tier fits. [Official Resend pricing](https://resend.com/pricing/).
- Budget approximately €6–€15/month for a small shared VM, plus an explicit allowance for encrypted off-host backups, storage growth, domain renewal and applicable taxes. Region/currency/IP options change the exact quote; inspect it at provisioning time. [Official Hetzner pricing changes](https://docs.hetzner.com/general/infrastructure-and-availability/price-adjustment/).
- Plan roughly **$15–$45/month equivalent before labor** as an initial operating allowance, not a provider quote or validated unit-economics claim. Track operator support/onboarding time separately; it will likely dominate a three-business pilot.
